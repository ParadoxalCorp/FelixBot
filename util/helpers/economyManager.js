...
class EconomyManager {
    ...
    constructor(client) {
        ...
        this.slotsEvents = [{
            ...
            changeRate: [-40, -60],
            conditionalVariants: [{
                condition: (userEntry) => this.user(userEntry).hasItem(1000),
                success: `A cat run into you and steals \`{value}\` holy coins from your gains ! But your ${this.getItem(1000).name} catch it and gets your gains back !`,
                fail: `A cat run into you and steals \`{value}\` holy coins from your gains ! But your ${this.getItem(1000).name} catch it and... wait, your ${this.getItem(1000).name} got beaten by the cat !`,
                successRate: 75
            }],
            case: 'won'
        }, {
     ...
     * @param {object} userEntry - The user database entry
     * @returns {userEntry} The given user entry with additional methods
     */
    user(userEntry) {
        return Object.defineProperties(userEntry, {
            hasItem: {
                value: (itemID) => {
                    return userEntry.economy.items.find(i => i.id === itemID) ? true : false;
                },
                writable: false
            }
        });
