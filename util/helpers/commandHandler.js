module.exports = async(client, message) => {
    ...
    if (!message.guild) {
        ...
        if (command.conf.guildOnly || (command.help.category === 'admin' && !client.config.admins.includes(message.author.id))) return await message.channel.send(":x: This command can only be used in a guild or you don't have the permission to use it");
        //Check if the command is disabled
        if (command.conf.disabled !== false) return await message.channel.send(":x: Sorry but this command is disabled for now\n**Reason:** " + command.conf.disabled);
        try {
            await command.run(client, message, args);
        } catch (err) {
            client.emit('fail', err, message);
        } finally {
            command.uses++;
            client.commandsUsed++;
        }
        ...
        //Disabled check
        if (command.conf.disabled) return message.channel.send(":x: Sorry but this command is disabled for now\n**Reason:** " + command.conf.disabled);
        //Permissions check
        const allowed = await require(`./permissionsChecker.js`)(client, message, command);
        if (!allowed) return await message.channel.send(":x: You don't have the permission to use this command !");
        ...
        try {
            ...
            if (command.shortcut && args.filter(a => command.shortcut.triggers.has(a.toLowerCase())).length > 0) {
                ...
                if (command.shortcut.triggers.get(trigger).args && (args.length - 1) < command.shortcut.triggers.get(trigger).args) return await message.channel.send(`:x: You didn't specified any or not enough arguments`);
