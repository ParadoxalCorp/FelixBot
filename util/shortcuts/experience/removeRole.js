module.exports = async(client, message, args) => {
    ...
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if (!guildEntry.levelSystem.roles.find(r => r.id === role.first().id)) return resolve(await message.channel.createMessage(`:x: That role isn't set to be given at any point`));
            guildEntry.levelSystem.roles.splice(guildEntry.levelSystem.roles.findIndex(r => r.id === role.first().id), 1);
