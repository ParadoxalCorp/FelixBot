module.exports = async(client, message, args) => {
    ...
    return new Promise(async(resolve, reject) => {
        try {
            ...
            userEntry.dataPrivacy.publicLove = privacy === 'public' ? true : false;
            client.userData.set(message.author.id, userEntry);
            resolve(await message.channel.createMessage(`:white_check_mark: The privacy of your love points has been successfully set to \`${privacy}\``));
        } catch (err) {
            reject(err);
        }
    });
