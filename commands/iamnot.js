...
exports.run = async(client, message) => {
    try {
        ...
        if (whitespace === -1) {
            ...
            if (!message.guild.member(message.author).roles.get(role.id)) {
                return await message.channel.send(':x: You dont have this role !');
            }
            message.guild.member(message.author).removeRole(role);
            return await message.channel.send(":white_check_mark: Alright, i removed from you the role **" + role.name + "**");
        }
    } catch (err) {
        var guild;
...
exports.help = {
    name: 'iamnot',
    description: 'Remove a self-assignable role from you',
    usage: 'iamnot nsfw',
    category: 'generic',
    detailledUsage: '`{prefix}iamnot` Will return the self-assignables roles list\n`{prefix}iamnot Neko` Will remove from you the role `Neko` if this role is self-assignable and if you have it'
};