...
class Trabslate extends Command {
    constructor() {
        super();
        this.help = {
            name: 'translate',
    ...
    async run(client, message, args) {
        args = args.join(" ").split('"').filter(a => a !== "").map(a => a.trim());
                ...
                if (!args[1] || !textToTranslate) return message.channel.createMessage(`:x: You need to at least specify the text to translate and the language to which i should translate it`);
