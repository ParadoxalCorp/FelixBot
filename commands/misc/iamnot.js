        ...
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (args.length < 1) {
                    ...
                    let listMessage = function(page, raw) {
                        return {
                            embed: {
                                ...
                                description: "Here's the list of the self-assignable role, you can remove one from yourself with `" + guildEntry.generalSettings.prefix + "iamnot [role_name]`\n" + (raw ? "```\n" + roleList[page][0].join(`\n`) + "```" : ""),
                                footer: {
                    ...
                    if (!message.guild.members.get(message.author.id).roles.find(r => r === guildRole.first().id)) return resolve(await message.channel.send(`:x: You don't have this role so i can't really remove it :v`));
                    await message.guild.members.get(message.author.id).removeRole(guildRole.first().id);
                    resolve(await message.channel.send(":white_check_mark: Alright, i removed from you the role `" + guildRole.first().name + "`"));
                }
            } catch (err) {
                reject(err);
            }
