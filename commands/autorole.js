...
exports.run = async(client, message) => {
    try {
        ...
        if ((add === -1) && (remove === -1)) {
            ...
            if (guildEntry.autoAssignablesRoles.indexOf(roleName.id) !== -1) {
                return await message.channel.send(":x: The role you specified is already a self-assignable role");
            }
            guildEntry.autoAssignablesRoles.push(roleName.id);
            fs.writeFile(client.dbPath, JSON.stringify(client.database), err => {
                if (err) console.error(err)
            });
...
exports.help = {
    ...
    category: 'settings',
    detailledUsage: 'By adding a role to the self-assignables roles list, users will be able to give themselves this role using `f!iam rolnename`\n`f!autorole -remove nsfw` Will remove the role `nsfw` from the list of self-assignables roles\n`f!autorole` Will return the list of self-assignables roles'
};
