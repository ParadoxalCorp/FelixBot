    ...
    run(client, message, args) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (message.guild && !message.guild.members.get(message.author.id).hasPermission("administrator") && !dm) {
                ...
                for (let i = 0; i < categories.length; i++) {
                    ...
                    overallHelp += `**${categories[i]}** =>` + categoryCommands.filter(c => message.guild ? (!client.guildData.get(message.guild.id).generalSettings.disabledModules.includes(c.help.name) || message.guild.members.get(message.author.id).hasPermission("administrator")) : true).map(c => `\`${c.help.name}\` `) + "\n\n";
                }
                overallHelp = overallHelp.replace(/undefined/gim, ""); //To remove the "undefined"
                //--------------------------------------------------------Return overall help if no args-----------------------------------------------------
                if (args.length === 0 || dm || allowed) {
