    ...
    run(client, message, args) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (args.length === 0) return resolve(await client.createMessage(message.channel.id, `Here's the list of all commands categories and their respective commands. Use \`${prefix}help command name\` to see the detailed description of a command\n\n` + overallHelp + `\n\n**Tips**\nYou can run up to 3 commands at once using \`&&\`, so for example: \`${prefix}ping && ${prefix}awoo\`\nIf you made a typo while writting a command, as long as its your last message, you can edit it: Felix will run it`));
                ...
                resolve(await client.createMessage(message.channel.id, `${commandHelp.help.description}\n**Parameters:** ${parameters}\n**Usage Example:**\n\`${prefix + commandHelp.help.usage}\`\n**Category:** \`${commandHelp.help.category}\`\n**Aliases:** \`${aliases}\`\n**Detailed usage:**\n${detailedUsage}\n**Shortcuts:**\n${shortcuts}`));
            } catch (err) {
                reject(err);
            }
        });
