...
exports.run = async(client, message) => {
    return new Promise(async(resolve, reject) => {
        try {
            fetch: {
                await unirest.get("https://api.weeb.sh/images/random?type=cuddle")
                ...
                .end(async function(result) {
                    ...
                    if (users.size > 0) {
                        resolve(await message.channel.send({
                            embed: {
                                description: `Hey ${users.map(u => u.toString()).join(", ")}, you just received a cuddle from ${message.author.toString()}`,
                                image: {
                                    url: cuddleUrl
                                },
                                footer: {
