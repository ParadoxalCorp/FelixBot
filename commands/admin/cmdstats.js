                ...
                if (!args[0]) {
                    ...
                    resolve(await message.channel.createMessage({
                        embed: {
                            ...
                            description: '```\n' + categories.map(c => `${c.name}: ${c.uses}/${c.totalUses} -|- ${(c.uses / client.commandsUsed * 100).toFixed(0)}%/${(c.totalUses / overallCommandsUsed * 100).toFixed(0)}%`).join('\n') + '```',
                            footer: {
                    ...
                    resolve(await message.channel.createMessage({
                        embed: {
                            ...
                            description: '```\n' + client.commands.filter(c => c.help.category === args[0]).map(c => `${c.help.name}: ${c.uses}/${c.totalUses} -|- ${(c.uses / client.commandsUsed * 100).toFixed(0)}%/${(c.totalUses / overallCommandsUsed * 100).toFixed(0)}%`).join('\n') + '```',
                            footer: {
                                text: `For a total of ${client.commandsUsed}/${overallCommandsUsed} commands used`
                            }
                        }
