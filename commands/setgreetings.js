...
exports.help = {
    ...
    category: 'generic',
    detailledUsage: '`f!setgreetings -dm Welcome` Will send the message "Welcome" to every new members in their private messages\n`f!setgreetings -channel Welcome` Will send the message "Welcome" to every new members in the channel you used the command\n`f!setgreetings -remove` Will remove the greetings message.\nYou can use `{user}` and `{server}` to add the new member name and your server name in the message, so \n`f!setgreetings -channel Welcome {user} to {server}`\n will look like: `Welcome @Baguette to Felix\'s lovers`\nYou can display the current greetings message by using `f!setgreetings`'
};