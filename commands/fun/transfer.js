    ...
    run(client, message, args) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (!args[0]) return resolve(await message.channel.createMessage(`You currently have **${userEntry.generalSettings.points}** points`));
                //Else continue the transfer process
                if (!users.first()) return resolve(await message.channel.createMessage(`:x: You did not specified any user or i couldn't find them :v`));
                let amount = args.filter(a => !isNaN(a) && a !== users.first().id); //Get the amount and make id resolvable work
                if (!amount[0]) return resolve(await message.channel.createMessage(':x: You did not specified the amount of points that i should transfer to them'));
                else if (amount[0] < 0) return resolve(await message.channel.createMessage(`:x: Ehhhhh, do you wanna steal **${users.first().username}**'s points or something? Thats baaaadddd`))
