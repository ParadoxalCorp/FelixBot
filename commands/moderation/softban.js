...
class Softban {
    constructor() {
        this.help = {
            name: "softban",
            usage: "softban <user_resolvable> <days_of_messages_to_delete> -r <reason>",
            description: "Softban a member from the server, its basically banning and unbanning instantly a member to deleta their messages. The reason is optional and can be added after. As for the number of days of messages to delete, this can be between 0 and 7."
        }
        this.conf = {
            guildOnly: true,
    ...
    run(client, message, args) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                await message.guild.members.get(memberToBan.first().id).ban(daysAmount, `Softbanned by ${message.author.tag}: ${reason ? reason : "No reason specified"}`);
