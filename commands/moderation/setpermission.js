    ...
    run(client, message, args) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                if (!action[0]) return resolve(await message.channel.createMessage(`:x: You did not specified whether i should enable or restrict \`${permission[0]}\``));
                //Set global permissions
                if (targets === 'global') {
                    if (action[0] === 'true') {
                        //If already set to this, return
                        if (guildEntry.permissions.global.allowedCommands.includes(permission[0])) return resolve(await message.channel.createMessage(`:x: \`${permission[0]}\` is already set to \`true\``));
                        ...
                        if (guildEntry.permissions.global.restrictedCommands.includes(permission[0])) return resolve(await message.channel.createMessage(`:x: \`${permission[0]}\` is already set to \`false\``));
                    ...
                    resolve(await message.channel.createMessage(`:white_check_mark: Alright, \`${permission[0]}\` has been set to \`${action[0]}\``));
                }
                ...
                //Set roles permissions
                    ...
                    client.guildData.set(message.guild.id, guildEntry);
                    resolve(await message.channel.createMessage(`:white_check_mark: Alright, \`${permission[0]}\` has been set to \`${action[0]}\` for the role(s) **${targets.map(c => c.name).join(', ')}**`));
                }
            } catch (err) {
                reject(err);
            }
