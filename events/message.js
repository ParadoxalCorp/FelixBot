...
module.exports = async(client, message) => {
    ...
    if (mentionned && client.userData.get(mentionned.id)) client.userData.set(mentionned.id, client.defaultUserData(mentionned.id));
    ...
    if (message.channel.type !== "dm") {
        ...
        if (message.content.startsWith(`<@${client.user.id}>`) || message.content.startsWith(`<@!${client.user.id}>`)) { //Stuff like @Felix prefix            
            ...
            if (request === "prefix") return message.channel.send("The current prefix on this server is **" + guildEntry.generalSettings.prefix + "**");
                ...
                if (message.guild) return await message.channel.send("Here's the commands list, you can see the detailed help of a command using `" + guildEntry.generalSettings.prefix + "help commandname`\n\n" + client.overallHelp);
                else return await message.channel.send("Here's the commands list, you can see the detailed help of a command using `" + client.database.prefix + "help commandname`\n\n" + client.overallHelp);
            }
        }
    }
    //Commands
